...
func CreateTemplateCache() (map[string]*template.Template, error) {
	...
	pages, err := fs.Glob(akademy.FS, "templates/*.page.html")
	if err != nil {
	...
	for _, page := range pages {
		...
		ts, err := template.New(name).Funcs(functions).ParseFS(akademy.FS, page)
		if err != nil {
			log.Printf("unable to ParseFiles: %v", err)
			return nil, err
		}
		matches, err := fs.Glob(akademy.FS, "templates/*.layout.html")
		if err != nil {
			log.Printf("unable to fs.Glob: %v", err)
			return nil, err
		}
		if len(matches) > 0 {
			ts, err = ts.ParseFS(akademy.FS, "templates/*.layout.html")
			if err != nil {
